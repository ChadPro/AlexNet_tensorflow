...
def fc_block(inputs, outputs, regularizer, activation=None, flatten=False, is_dropout=False, is_training=True, scope=""):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        if regularizer != None:
            tf.add_to_collection("regularizer_losses", regularizer(fc_weights))
...
def alexnet_net(inputs, \
    ...
    if white_bal:
        img_scaled = dst_img
        blue, green, red = tf.split(axis=3, num_or_size_splits=3, value=img_scaled)
