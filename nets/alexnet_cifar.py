"""*************************************************************************
	> File Name: alexnet_32.py
	> Author: LongYan
	> Created Time: 2019年02月24日 星期日 10时54分42秒
 ************************************************************************"""
 # -- coding: utf-8 --
...
NUM_CHANNELS = 3
STDDEV = 0.01
CIFAR10_MEAN = [114.030, 123.076, 125.426] #bgr
...
def alexnet_net(inputs, \
    ...
    if white_bal:
        ...
        dst_img = tf.concat(axis=3, values=[blue - CIFAR10_MEAN[0],green - CIFAR10_MEAN[1],red - CIFAR10_MEAN[2],])
    ...
    with tf.name_scope(scope):
        ...
        net = maxpool_block(net, [1,2,2,1], [1,2,2,1], is_training=is_training, scope="pool_3")
